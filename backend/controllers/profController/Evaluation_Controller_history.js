...
exports.all_evaluation_Controller = async (req, res) => {
  try {
    ...
    if (result.length > 0) {
      ...
      // No evaluations found → 204 No Content
...
exports.search_by_id_evaluation_Controller = async (req, res) => {
  try {
    ...
    const id = req.user.id;
    const evalId = parseInt(id_evaluation, 10);

    const result = await EvaluationModel.search_by_id_evaluation_Model(evalId, id);
...
exports.filter_by_level_Controller = async (req, res) => {
  try {
    ...
    if (result.length > 0) {
      ...
      return res
        .status(404)
        .json({ message: "No data found for this level." });
    }
  } catch (error) {
    console.error(error);
    return res
...
exports.filter_by_class_Controller = async (req, res) => {
  try {
    ...
    if (result.length > 0) {
      ...
      return res
        .status(404)
        .json({ message: "No data found for this class." });
    }
  } catch (error) {
    console.error(error);
    return res
...
exports.filter_by_type_Controller = async (req, res) => {
  try {
    ...
    if (result.length > 0) {
      ...
      return res
        .status(404)
        .json({ message: "No data found for this type." });
    }
  } catch (error) {
    console.error(error);
    return res
...
exports.view_evaluation_Controller = async (req, res) => {
  try {
    ...
    // Assuming the model returns an object like { comment, result }
    const { comment, result } = await EvaluationModel.view_evaluation_Model(evalId);
