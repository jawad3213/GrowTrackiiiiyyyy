  ...
  <div v-if="isOpen" class="fixed inset-0 bg-gray-800/70 backdrop-blur-sm flex items-center justify-center z-50 font-inter">
    <div class="bg-white border-2 border-purple-500 rounded-2xl shadow-xl w-full max-w-2xl p-6 sm:p-8">
      ...
      <form @submit.prevent="submitForm" class="space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label class="text-sm font-medium text-gray-700">Date of beginning*</label>
            <input v-model="supervisor.dateStart" type="date"
              class="w-full px-4 py-2 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-purple-500" />
            <p v-if="errors.dateStart" class="text-red-500 text-xs mt-1">{{ errors.dateStart }}</p>
          </div>
          <div>
            <label class="text-sm font-medium text-gray-700">Date of end*</label>
        ...
        <div>
          ...
          <textarea v-model="supervisor.subject" rows="3" placeholder="e.g. I joined..."
        ...
        <div class="flex justify-between pt-4">
          ...
          <button type="submit"
            class="px-8 py-2 font-semibold text-white rounded-md bg-gradient-to-r from-purple-600 to-purple-500 hover:from-purple-700 hover:to-purple-600 transition-transform duration-300 hover:scale-105">
            Confirm
          </button>
        </div>
