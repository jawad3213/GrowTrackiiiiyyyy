...
const fetchSolution = async () => {
  try {
    const response = await axios.get(`http://localhost:3001/studentsolutions?signalId=${Number(route.query.id)}`)
    ...
    errorMsg.value = err.response?.data?.message || 'error occured please try again'
